...
const percentageLabel = (value: number, total?: number | null) => total && total > 0
  ...
  : '—';
const publishedBlankCount = (scrutiny: Scrutiny) => scrutiny.status === 'published'
  ? Math.max(0, (scrutiny.ballotCount * scrutiny.maxMarks) - scrutiny.results.reduce((sum, result) => sum + result.votes, 0))
  : scrutiny.blankCount;
...
function ScrutinyResultCard({ scrutiny, presentMembers, embedded = false }: { scrutiny: Scrutiny; presentMembers?: number | null; embedded?: boolean }) {
  const blankCount = publishedBlankCount(scrutiny);
  return <article className={`panel scrutiny-result ${embedded ? 'scrutiny-result--embedded' : ''}`}>
    ...
    <footer><span><strong>{blankCount}</strong> {blankCount === 1 ? 'voto em branco' : 'votos em branco'}</span><span>{countLabel(scrutiny.digitalCount, 'digital', 'digitais')} · {scrutiny.paperCount} em papel</span></footer>
  </article>;
}
...
function CurrentPartialResults({ election }: { election: ElectionSummary & { presentMembers?: number | null } }) {
  ...
  function resultRow(candidate: (typeof candidates)[number]) {
    ...
    return <div className={candidate.elected ? 'is-elected' : candidate.declined ? 'is-declined' : ''} key={candidate.id}>
      <div className="result-candidate"><span>{candidate.name}{candidate.electedRound && <em>Eleito no {candidate.electedRound}º</em>}{candidate.declinedRound && <em className="declined-label">Não aceitou</em>}</span><i><b style={{ width: `${percentage}%` }} /></i></div>
...
function ElectionControl({ election, refresh }: { election: Election; refresh: () => Promise<void> }) {
  ...
  function renderAdminTimelineStep(step: TimelineStep) {
    ...
    if (closed && tally && step.key === `${closed.office}-${closed.roundNumber}`) return <section className="panel stack timeline-action-panel"><div><p className="eyebrow">Conferência da mesa</p><h2>Apuração do {closed.roundNumber}º escrutínio</h2></div>
      ...
      <div className="tally-table">{tally.candidates.map((candidate) => <div key={candidate.id}><span>{candidate.name}{winners.includes(candidate.id) ? <small>Dentro das vagas</small> : tally.cutoffTie?.candidateIds.includes(candidate.id) ? <small>Empatado no corte</small> : candidate.qualified ? <small>Atingiu a maioria</small> : null}</span><strong>{candidate.votes}</strong></div>)}</div>
      ...
      </section>{tally.cutoffTie && <div className="message message--warning cutoff-tie-message"><strong>Empate no limite das vagas.</strong><span>{tally.cutoffTie.candidateIds.length} candidatos empataram com {tally.cutoffTie.votes} voto(s) por {tally.cutoffTie.seatCount} vaga(s). {closed.roundNumber < 3 ? 'Eles continuarão elegíveis no próximo escrutínio.' : 'Como este é o terceiro escrutínio, a vaga permanecerá aberta.'}</span><div className="cutoff-tie-names">{tally.cutoffTie.candidateIds.map((candidateId) => <span key={candidateId}>{tally.candidates.find((candidate) => candidate.id === candidateId)?.name}</span>)}</div></div>}
      ...
      <div className="action-footer"><p>{winners.length > 0 ? 'Confirme todas as respostas antes de publicar.' : tally.cutoffTie && closed.roundNumber < 3 ? 'Publique a apuração para levar o empate ao próximo escrutínio.' : 'Confira a apuração e publique para liberar a próxima etapa.'}</p><button className="button button--primary" disabled={busy || !acceptanceComplete} onClick={() => act(() => request(`/admin/scrutinies/${closed.id}/publish`, { method: 'POST', body: JSON.stringify({ winnerIds: winners, acceptances: winners.map((candidateId) => ({ candidateId, accepted: acceptances[candidateId] })) }) }, true), 'Resultado publicado. A próxima etapa já está disponível na timeline.')}>Aprovar e publicar resultado</button></div>
    </section>;
